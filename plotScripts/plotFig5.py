...
tempFactor_dir = "/div/qbo/users/anenj/GTDR/calcSensitivityFactors/concFactors/"
...
axs[0].set_ylabel("[1/ppm]",font={"size":15})
axs[1].set_ylabel("[1/ppm]",font={"size":15})
axs[2].set_ylabel("[1/ppm]",font={"size":15})
axs[3].set_ylabel("[1/ppm]",font={"size":15})
axs[0].set_ylim([-0.001,0.003])
axs[1].set_ylim([-0.001,0.003])
axs[2].set_ylim([-0.001,0.003])
axs[3].set_ylim([-0.001,0.003])
...
for n,ax in enumerate(axs.flat):  
    ax.text(-0.135, 0.92, letters[n], transform=ax.transAxes, 
            size=20, weight='bold')

fig.legend(bbox_to_anchor=(0.176,0.93),loc="upper left",fontsize=15,ncol=3)

fName = "plots/figure5.png"
plt.savefig(fName)
#cmd = "display "+fName+" &"
#os.system(cmd)
