...
for p,prod in enumerate(prodList):
    ...
    for idx in index:
        ...
        if reg=="AU" and prod=="Oil seeds":
            ...
            axs[i,j].scatter(x*100,y*100,label=reg,color=colormap(df_tempScale["tas_norm"].loc[reg]))
            xList.append(x)
            yList.append(y)
    # *******************************
    ...
    axs[i,j].set_title(prod,x=0.15,y=0.,fontsize=15)
    if prod=="Cereal grains nec":
        axs[i,j].set_title(prod,x=0.25,y=0.,fontsize=15)
    elif prod=="Paddy rice":
        axs[i,j].set_title(prod,x=0.15,y=0.,fontsize=15)
    elif prod=="Wheat":
        axs[i,j].set_title(prod,x=0.1,y=0.,fontsize=15)
    elif prod=="Oil seeds":
        axs[i,j].set_title(prod,x=0.14,y=0.,fontsize=15)
    if i==0:
        i+=1
    else:
        j+=1
...
cmap.set_label("Countries distributed on colormap based on their temperature\n average over 10 years, temperature [$\!\!^\circ\!\!$C] indicated on the colorbar.", rotation=270, labelpad=65,fontsize=15)
...
for n,ax in enumerate(axs.flat):  
    ax.text(0.03, 0.9, letters[n], transform=ax.transAxes, 
            size=20, weight='bold')

plt.savefig("plots/figure_7.png")
